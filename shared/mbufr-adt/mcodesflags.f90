...
! 20250828 - SHSF - the reading format of C-13 tables has been updated to read the table from WMO website
 use stringflib
...
!-------------------
	...
	CTable_name=trim(mbufr_tables)//"C13.csv"
	open(2,file=CTable_name,status="unknown")
	tabCC13(:,:)=""
...
711		read(2,'(a)',end=712)line
		...
		!   PRINT *,BTYPE,BSUBTYPE,TABCC13(BTYPE,BSUBTYPE)
		   btype=val2(tab(1))
		   bsubtype=val2(tab(3))
		   tabcc13(btype,bsubtype)=trim(tab(4))
		 end if 
