...
! 20250410 V4.7.3 SHSF - Use of integer(8) in the representation of RGMAX 
! 20250410 V4.7.4 SHSF - Consider the case of a empty section 4 in the message
! 20250815 V4.7.4 SHSF - Consider error 15 related to Version of Local BUFR table
! 20250831 V4.7.5 SHSF - Inclusion of Error code 58 and revision of the compressed data reading with delayed replication
! 20260125 V4.7.5B SHSF- Small changes in the undef value function. It must be checked later
!----------------------------------------------------------------------------------------------------------------
...
!{ Declara como PUBLICAS os seguintes tipos de estruturas
  ...
  INTEGER,PARAMETER :: intk  = 8 !selected_int_kind (8)  !< used in binary conversions  8 bytes -> ~Maximum 9,223,372,036,854,775,807
  INTEGER,PARAMETER :: intk2 = 16
  integer,parameter :: AutoFix_mode=1
 !INTEGER,PARAMETER :: intk2 = 8  !INTEGER*4 (32bits) range (-2147483648, 2147483647),
                                  !Integer*8 (64 bits) range (-9223372036854775808,9223372036854775807).Usar isto 4611686018427387904 Usar opcao -fno-range-check
   integer           ,parameter :: currentRGMax=2147483640
  !integer(kind=intk),parameter  :: currentRGMax=4611686018427387904
...
!}
  ...
  CHARACTER(LEN=25),PARAMETER                     ::MBUFR_VERSION=" 4.7.5b 2026-01-21      "
	...
	!{
	  ...
	  IF (((A_VAL(n)<0).or.(A_VAL(n)>valmax)).and.valmax>0) THEN
		...
		IF ((index(SUBNAME,"SAVESEC4")>0).or.(index(SUBNAME,"SAVESEC1")>0))  THEN 
		  A_VAL(n)=valmax
		ELSE
	  	  PRINT *,"Fatal Error 1 in "//TRIM(SUBNAME)//"_put_octets !"
...
if (noct>ubound(oct,1)) then 
	print *,"Fatal Error 2  in "//TRIM(SUBNAME)//"_put_octets !"
	stop
end if
do j=1,noct
  v = 0 
 ...
 10 continue
  ...
  !{ 
   ...
   if (present(optsec)) then 
    ...
    if (errsec==32) then  
       warning=32 ! section4 without subsets 
       errsec=0
    end if 
...
!{
   ...
   !just in case of empty section 4 (withou subsets)  
    if (warning==32) then 
        if (verbose>0) print *," :MBUFR-ADT: Warning! number of subsets = 0 in section 4"
        sec3%nsubsets=0
        sec4%nvars=0
        errsec=0
        goto 99
...
!}
 ...
 !------------------------------------------------------------	  
 !{ 
	if (IFinal<nvars) then
	   Delayed_Replicator=.true.
    ELSE
       Delayed_replicator=.false.
    end if
 ...
 !-----------------------------------------------------------------------
   ...
   !{
    ...
    ELSEIF ((sec3%is_cpk/=0).and.(IFinal<nvars)) THEN
     ...
     !{
        RGSEC4=CurrentRG
        if (verbose>2) print *," :MBUFR-ADT: runing Readsec4CMP with Deleyed replicators"
        call READSEC4CMP(un,desc_sec4,ifinal,sec3%nsubsets,sec4,tam_sec4,errcmp)
     ...
     ! normal
      !{
        ...
        if ((Fator_Replicacao>999).and.(Autofix_mode>0)) then
           print *," :MBUFR-ADT: Error 58: ",ERROMESSAGE(58)
           errsec=58
           nvars=ifinal-2 ! Sequence finalized by error
           goto 98
        end if
      ...
      !do i=ifinal-2,ifinal
      ! print *,i,ifinal,write_descriptor(desc_sec4(i))
      !end do
      errsec=err
      if (err==0) goto 70
      !} 
    ...
    !}
     ...
     !---------------------------------------------------------------
     !{
 98   IF ((errsec>0).and.(errsec<60)) THEN
	   do j=1,sec3%nsubsets
		...
		END do
	   END do
       if (verbose>2) call ERRORLOG(un,errsec,desc_sec4,nvars, erromessage(errsec))
     END IF
   !}
...
50   tam_sec3max=NBYTES-tam_sec2-30
      call READSEC3(un,tam_sec3max,desc_sec3,sec3,tam_sec3,err)
        ...
        if (err==32) then 
            ...
            if (verbose>2)  write(*,'(" :MBUFR-ADT: Warning ",i2,": ",a)')err,trim(erromessage(err))
	    errsec=32   
	    goto 60
        end if 
	IF (err/=0) THEN
...
!{ Reinicia tabela se necessario  
    if (reinittab) then 
      ...
      if (err>0) then 
        ...
        if (verbose>0) then
        !Display  Error 14 or Error 15
        write(*,'(":MBUFR_ADT: Error ",i2,"! ",a)')check_vertables,ERROMESSAGE(check_vertables)
        write(*,'(":MBUFR_ADT: Tables B",a10,".txt or/and D",a10,".txt Not Found")')auxc,auxc
...
!{ Decoficacao dos 7 primeiros bits
 ...
 !}
 	...
 	IF(sec3e%nsubsets<1) THEN 
		ERR_sec3=32
		! Number of subsets in section 4 is lese than 1
		!deallocate (a,b,sec3b)
		!return
		sec3e%nsubsets=0
	END IF
...
! 
 ...
 do while (j<ndesc2)
    ...
    !-----------------------------------------------------------
    ! Obtendo o numero de bits de cada descritor da tabela B (j)
    ! Geting the bit width of each table B descriptor (j)
    !---------------------------------------------------------
    !{
      IF (d(j)%f==0) THEN
          ...
          if (d(j)%i==0) then 
              ...
              IF (ERR < 0) then
                  if (verbose>2) then
                      print *," :MBUFR-ADT: Warning! Error reading and decompressing sec4 data (1) "
                      print *," :MBUFR-ADT: Descriptor ",write_descriptor(d(j))," pos=", j
		          end if
                  ndesc2=j
              end if
              vmini=a(k)   !' Minimum value
           !}
           ...
           !{ Leitura do contador de bits (bit counter)
              ...
              IF (ERR < 0) THEN 
                 ...
                 if (verbose>1) then
                    ...
                    print *,"Descriptor [",j,"] = ",write_descriptor(d(j))
                 end if
                 ndesc2=j
                 bbit=0
              else
             ...
             !{ ***Verifica consistencia do valor minimo e do contador de bits
              ...
              if (bbit<0) then
                  bbit=0
                  if (verbose>2) then
                      print *," :MBUFR-ADT: Warning!  Error reading and decompressing sec4 data (2) "
                      print *," :MBUFR-ADT: Bit counter = null "
                      print *," :MBUFR-ADT: Descriptor ",write_descriptor(d(j))," pos=", j
                  end if
                 ndesc2=j
               END IF
               ...
               if ((vmini<0).and.(bbit>0)) then
                  !errsec4=1
                  if (verbose>2) then
                      print *," :MBUFR-ADT: Warning! Unexpected values reading and decompressing sec4 data (3) "
                      print *," :MBUFR-ADT: Minimum Value  = null and bit counter >0)"
                      print *," :MBUFR-ADT: Descriptor ",write_descriptor(d(j))," pos=", j
                  end if
                  bbit=0
              end if          
...
!}
  ...
  IF ((desc(idesc)%f==1).and.(desc(idesc)%x/=0)) THEN
      ...
      j=0
       ...
       ! O primeiro descritor a ser replicado e chamado descritor primeiro filho 
       ! e o padramentro k=.true. Os demais sao descritores irmaos (k=.false.)
       !-------------------------------------------------------------------------
       !{
       IF(dc(i)%y>0) THEN   
        jy=dc(i)%y
	 ...
	 !{
      ...
      ELSE
        ...
        IF ((desc(idesc+1)%f/=0).and.(desc(idesc+1)%x/=31)) THEN 
          err=56  ! Erro na utilização de replicador delayed
          print *," :MBUFR-ADT:replicdesc: ERROR 56 !",trim(ERROMESSAGE(56))
	  
	  !{
	
	     print *,"         Center............",cur_sec1%center
	     print *,"         subcenter.........",cur_sec1%subcenter
	     print *,"         BUFR Category.....",cur_sec1%btype
	     print *,"         BUFR SubCategory..",cur_sec1%bsubtype
             print *,"         message number....",NMSG
	     print *,"         Descriptors:"
	     !{
            do i=idesc,Idesc+2
              write(*,'(22X,i5," -> ",i1,"-",i2.2,"-",i3.3)')i,desc(i)%f,desc(i)%x,desc(i)%y
            end do
            !}  
...
!{
 function vmax_numbits(nbits);INTEGER(kind=intk2)::vmax_numbits
	  ...
	  INTEGER(kind=intk2),parameter::d=2,u=1
	   vmax_numbits=(d**nbits)-u
          ! Subtrai um para transformar 1000000 -> 01111111
          ! Nota( nao considera o bit de indicacao de sinal)
  END function vmax_numbits

  integer(kind=intk2) function integer_limit(nbytes)
    integer(kind=intk2),intent(in)::nbytes
    integer(kind=intk2)::nbits
    integer(kind=intk2),parameter::o=8,d=2,u=2
    nbits=o*nbytes
    integer_limit=(d**(nbits-u))
...
!------------------------------------------------------------------------------!
! kind = intk was replaced by kind=4 .  This needs to be checked later.
         function vmax_int();integer(kind=4)::vmax_int
            integer(kind=4)::r
            integer::i
            i=digits(r)
...
SUBROUTINE INIT_ERROMESSAGE
    ERROMESSAGE(:)=""
    !
    ERROMESSAGE(01)="Maximum file addressing was exceeded"
    ...
    ERROMESSAGE(15)="Error reading Local BUFR Tables"
    ...
    !{ Codigo de erros relativo a expansao dos descritores
      ...
      ERROMESSAGE(56)="Error in a delayed replicator or replicator factor"
      ERROMESSAGE(57)="Error in replication processing"
      ERROMESSAGE(58)="Error in Replication Factor"
...
10   nm=nm+1
	if (nm<possize) then 
		...
		if ((errsec>0).and.(errsec/=20).and.(errsec/=32)) then 
			nerrors=nerrors+1
...
!   7777
 ...
 function write_descriptor(desc); character(len=8)::write_descriptor
   TYPE(descbufr),intent(in)::desc
   write(write_descriptor,'(i1,"-",i2.2,"-",i3.3)')desc%f,desc%x,desc%y
 end function
